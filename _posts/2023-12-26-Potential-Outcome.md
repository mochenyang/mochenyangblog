...
toc: true
layout: post
categories: [exposition, causal-inference]
...
The interpretation of $\beta_{LR}$ is straightforward: $\beta_{LR} = \mathbb{E}(Y_i(D_i) | D_i = 1) - \mathbb{E}(Y_i(D_i) | D_i = 0) = \mathbb{E}(Y_i(1) - Y_i(0)).$ This quantity is **ATE** by definition, which can be readily estimated by a linear regression in a randomized experiment without non-compliance.
